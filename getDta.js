 ...
 *
 * - If aggregationType is "daily" (or not provided), it queries the Metrices table for raw daily data.
 * - If aggregationType is "weekly", "monthly", or "yearly", it queries the Aggregated table via the GSI "byAggregationType"
 *   to return aggregated data for all databases.
 */
export const handler = async (event) => {
  try {
    ...
    if (event.body) {
      ...
      if (event.queryStringParameters.aggregationType) {
        aggregationType = event.queryStringParameters.aggregationType;
      }
    }
